...
[DFT_ABS_noised1, DFT_ANG_noised1] = fanalyse(ecg_noised1(ecg_T), length(ecg_T), fs, verbose);
...
lowpass_designfilt = designfilt('lowpassfir', 'PassbandFrequency', 10, 'StopbandFrequency', 25, 'PassbandRipple', 1, 'StopbandAttenuation', 60, 'SampleRate', 512);
...
[DFT_ABS_noised2, DFT_ANG_noised2] = fanalyse(ecg_noised2(ecg_T), length(ecg_T), fs, verbose);
...
[DFT_ABS_noised3, DFT_ANG_noised3] = fanalyse(ecg_noised3(ecg_T), length(ecg_T), fs, verbose);
...
subplot(5, 1, 3);
plot(ecg_t_single_periode, ecg_noised2_lowpass_filtered(ecg_T), 'color', color_noised3);
grid minor
title('Lowpass filtered, non-shifted signal');
